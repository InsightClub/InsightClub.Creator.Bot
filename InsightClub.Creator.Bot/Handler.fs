...
let removeKeyboard config userId messageId =
    async {
        do!
...
let sendMessage config userId text keyboard =
    async {
        return!
...
let sendContent config userId storage content =
    async {
        let getFile fileId =
            async {
                let! fileStream = Storage.getFile storage fileId
...
let onUpdate connectToDb connectToStorage ctx =
    async {
        ...
        | { Message = Some ({ From = Some user } as message) } ->
            ...
            let services =
                Services.get connection config storage creatorId
            ...
            let renderServices =
                getRenderServices connection creatorId
