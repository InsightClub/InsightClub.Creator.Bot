...
type Apply<'Param, 'Result> = ('Param -> 'Result) -> 'Result

type Apply<'Result> = Apply<unit, 'Result>

type Services<'Result> = {
    tryCreateCourse: CourseTitle -> Apply<Result<CourseId, TitleError>, 'Result>
    tryUpdateTitle: CourseId -> CourseTitle -> Apply<Result<unit, TitleError>, 'Result>
    tryUpdateDesc: CourseId -> CourseDesc -> Apply<bool, 'Result>
    checkAnyCourses: Apply<bool, 'Result>
    getCoursesCount: Apply<Count, 'Result>
    tryCreateBlock: CourseId -> Index -> BlockTitle -> Apply<BlockId option, 'Result>
    addContent: BlockId -> Content -> Apply<'Result>
    getBlockInfo: BlockId -> Apply<Index * BlockTitle, 'Result>
    getBlocksCount: CourseId -> Apply<Count, 'Result>
    checkAnyBlocks: CourseId -> Apply<bool, 'Result>
    getBlockContents: BlockId -> Apply<Content list, 'Result>
    getBlockInfoByIndex: CourseId -> Index -> Apply<BlockId * BlockTitle, 'Result>
    cleanBlock: BlockId -> Apply<bool, 'Result>
}
...
let private updateInactive =
    function
    | Some Inactive.Start -> Idle Idle.Started

    | None -> Inactive

let private updateIdle return' services =
    function
    | Some Idle.Help ->
...
let private updateCreatingCourse return' services =
    function
...
let private updateEditingCourse return' services courseId =
    function
...
let private updateEditingTitle return' services courseId =
    function
...
let private updateEditingDesc return' services courseId =
    function
...
let private updateListingCourses return' services (subState: ListingCourses.State) =
    function
    ...
    | Some (ListingCourses.Prev beginningReached) ->
        if subState.Page = 0 then
            let newState =
                ListingCourses { subState with Msg = ListingCourses.Started }
    ...
    | Some (ListingCourses.Next endingReached) ->
        services.getCoursesCount
            <| fun coursesCount ->
                if (subState.Page + 1) * subState.Count >= coursesCount
                then
                    let newState =
                        ListingCourses { subState with Msg = ListingCourses.Started }
    ...
    | None ->
        let newState =
            ListingCourses { subState with Msg = ListingCourses.Error }

        return'.withNoEffects newState

let private updateCreatingBlock return' services (subState: CreatingBlock.State) =
    function
        ...
        services.tryCreateBlock subState.CourseId subState.Index title
            <| fun blockId ->
                let newState =
                    ...
                    | None ->
                        CreatingBlock { subState with Msg = CreatingBlock.TitleReserved }

                return'.withNoEffects newState

    | None ->
        let newState =
            CreatingBlock { subState with Msg = CreatingBlock.Error }
...
let private updateEditingBlock return' services (subState: EditingBlock.State) =
    ...
    | Some (EditingBlock.Prev beginningReached) ->
        if subState.Index = 0 then
            let newState =
                EditingBlock { subState with Msg = EditingBlock.Started }
    ...
    | Some (EditingBlock.Clean blockEmpty) ->
        services.cleanBlock subState.BlockId
            <| fun cleaned ->
                let newState =
                    EditingBlock { subState with Msg = EditingBlock.Cleaned }
    ...
    | Some (EditingBlock.AddContent content) ->
        services.addContent subState.BlockId content
            <| fun () ->
                let newState =
                    EditingBlock { subState with Msg = EditingBlock.ContentAdded content }

                return'.withNoEffects newState

    | None ->
        let newState =
            EditingBlock { subState with Msg = EditingBlock.Error }

        return'.withNoEffects newState

let private updateListingBlocks return' services (subState: ListingBlocks.State) =
    function
    ...
    | Some (ListingBlocks.Prev beginningReached) ->
        if subState.Page = 0 then
            let newState =
                ListingBlocks { subState with Msg = ListingBlocks.Started }
    ...
    | Some (ListingBlocks.Next endingReached) ->
        services.getBlocksCount subState.CourseId
            <| fun blocksCount ->
                if (subState.Page + 1) * subState.Count >= blocksCount
                then
                    let newState =
                        ListingBlocks { subState with Msg = ListingBlocks.Started }
    ...
    | None ->
        let newState =
            ListingBlocks { subState with Msg = ListingBlocks.Error }

        return'.withNoEffects newState

let private updateLocal return' dispatcher services =
    function
    | Inactive ->
        let newState =
